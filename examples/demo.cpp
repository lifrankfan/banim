...
int main() {
    ...
    // Create ALU block
    auto alu = std::make_shared<Block>(GridCoord(14, 6), 3.0f, 4.0f, "ALU");
    ...
    // Create Memory block
    auto memory = std::make_shared<Block>(GridCoord(8, 2), 3.0f, 3.0f, "MEMORY");
    ...
    auto reg = std::make_shared<Block>(GridCoord(8, 12), 2.0f, 2.0f, "REG");
    ...
    auto instruction = std::make_shared<Text>(GridCoord(1, 14), "Press 'G' to toggle grid", 14.0f);
