...
int main() {
    ...
    // Title - No scene parameter needed!
    auto title = std::make_shared<Text>(GridCoord(5, 1), "Grid System Demo", 24.0f);
    title->setColor(1.0f, 1.0f, 1.0f);
    scene.add(title);

    // Create rectangles that are exactly 1 grid cell in size - Much cleaner!
    auto rect1 = std::make_shared<Rectangle>(GridCoord(0, 0), 1.0f, 1.0f);
    rect1->setColor(0.8f, 0.2f, 0.2f, 1.0f);
    scene.add(rect1);

    auto rect2 = std::make_shared<Rectangle>(GridCoord(4, 3), 1.0f, 1.0f);
    rect2->setColor(0.2f, 0.8f, 0.2f, 1.0f);
    scene.add(rect2);

    auto rect3 = std::make_shared<Rectangle>(GridCoord(6, 3), 1.0f, 1.0f);
    rect3->setColor(0.2f, 0.2f, 0.8f, 1.0f);
    scene.add(rect3);

    // Create circles that fill exactly 1 grid cell - Much cleaner!
    auto circle1 = std::make_shared<Circle>(GridCoord(2, 5), 0.5f, 0.5f);
    circle1->setColor(0.8f, 0.8f, 0.2f, 1.0f);
    scene.add(circle1);

    auto circle2 = std::make_shared<Circle>(GridCoord(4, 5), 0.5f, 0.5f);
    circle2->setColor(0.8f, 0.2f, 0.8f, 1.0f);
    scene.add(circle2);

    auto circle3 = std::make_shared<Circle>(GridCoord(6, 5), 0.5f, 0.5f);
    ...
    // Move rectangles - No scene parameter needed!
    ...
    // Instructions - No scene parameter needed!
    auto instruction1 = std::make_shared<Text>(GridCoord(5, 6.5f), "Press 'G' to toggle grid", 16.0f);
    instruction1->setColor(0.9f, 0.9f, 0.9f);
    scene.add(instruction1);
    
    auto instruction2 = std::make_shared<Text>(GridCoord(5, 7.2f), "Resize window - grid stays fixed!", 16.0f);
